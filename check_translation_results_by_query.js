...
async function getTranslations(minId, maxId) {
    let connection;
    try {
        connection = await pool.getConnection();
        let query = 'SELECT * FROM scene_default_phrases  WHERE id >= ? AND id <= ?';
        const params = [minId, maxId];
            ...
            messages: [
                {
                    ...
                    content: `你是一个严格的字符集检测助手。
                        ...
                        You must answer strictly in the following JSON format: {
                            "msg": "请写出文字检查的分析过程。第一步：严格检查所有文字的外观字形；第二步：判断是否所有文字均属于【${sourceLang}】的常规字符集；第三步：根据前两步得出结论。绝不能因为文本提及其他语言而误判。",
                            "content": 0 or 1
                        }
...
async function main() {
    ...
    try {
        ...
        // 先查询数据 1453 1585 checked
        // 先查询数据 1586 1873 checked
        const results = await getTranslations(1901, 2000);
        ...
        if (Array.isArray(results)){
            ...
            for (let m = 0; m < results.length; m++) {
                ...
                let navName = langMap[row.native_language]
                let tarName = langMap[row.target_language]
