...
async function queryByInfo(id) {
    let connection;
    try {
        connection = await pool.getConnection();
        const [rows] = await connection.query(`SELECT * FROM scene_default_phrases where id = ${id}`);
        return rows;
    } catch (error) {
...
async function updateData(sceneId, nativeLanguage, targetLanguage, phrases, intentions, defaultPhrases, pair1, pair2) {
  let connection;
  try {
    connection = await pool.getConnection();
    const [result] = await connection.query(
      `UPDATE scene_default_phrases 
       SET phrases = ?, intentions = ?, default_phrases = ?, pair1 = ?, pair2 = ?
       WHERE scene_id = ? AND native_language = ? AND target_language = ?`,
      [phrases, intentions, defaultPhrases, pair1, pair2, sceneId, nativeLanguage, targetLanguage]
    );
    return result;
  } catch (error) {
...
async function start (id){
    try {
        ...
        let newRows = await queryByInfo(id)
        if (newRows.length){
            console.log('数据已更新：：：：：',  JSON.stringify(newRows, null, 2));
        }
    } catch (error) {
...
async function main() {
    try {
        let ids = [2762,2767,3062,3067,3295,3523,3746,3751,4370,4375,4382,4387,4394,4399,4406,4411];

        for (let s = 0; s < ids.length; s++) {
            
            let id = ids[s];
            // 查询
            let rows = await queryByInfo(id)
            if (rows.length) {
                let cur = rows[0]
                sceneId = cur.scene_id;
                targetLanguage = cur.target_language;
                nativeLanguage = cur.native_language;
